...
#show: ieee.with(
  ...
  abstract: [
    ...
    Initial performance analysis was conducted in comparison to a reference which uses Constant Coding and Modulation (CCM) and optimises for maximum availability. It showed a significant uplift in data throughput over the course of the mission. Additionally, resource analysis of the target FPGA SoC and the implemented design, as well as timing analysis show that the system will be implementable in hardware.
  ],
  authors: (
    (
...
There are several dynamic factors inherit in satellite to ground communications, including pointing losses, total slant range and scintillation effects. Some of these factors can be known in advance, however others such as interference, are difficult to predict and have an outsized impact on error rates. For this reason systems with #acr("ACM") can improve link availability and spectral efficiency compared to #acr("CCM") or those that change based on predicted values but without adaptation, #acr("VCM"), however this does come at the cost of increased complexity.
...
#figure(
  table(
    ...
    [Antenna VSWR], [$1.9:1$], [ISIS Antenna Datasheet @isispace_cubesat_nodate],
    // ISIS ant datasheet
...
#figure(
  table(
    ...
    align: (left, center, center),
    table.header([*Name*], [*Values*], [*Source*]),
...
Due to the location of the ground station, the observed elevations did not rise above 40#sym.degree, as shown in @img:elev_times. The majority of passes occured in the range of 10#sym.degree to 20#sym.degree. The corresponding optimal modulation and coding rates and datarates are shown in @img:elev2modcod.

For reference, the data rate for a #acr("CCM") #acr("DVB-S2") system was used that maximised availability, in this case using QPSK 1/3 for the duration of the mission. The #acr("ACM") system was assumed to take the optimal setting for each 10 second timestep of the simulation.
...
Additionally, the bandwidth calculations did not account for filter roll-off, this is discussed further in.


= Transmitter Design and Implementation;

== Design
Following the ACM analysis, it was decided to implement a DVB-S2 compliant system with ACM capabilities. As flight hardware could not be sourced for this investigation, the target platform was a combination of a Digilent ZedBoard Zynq 7020 development board attached to an Analog Devices AD-FMCOMMS4 AD9364 development board. As the interface of the AD936x series transceivers are the same, the system could be verified with a different transceiver in the same family.
...
The FPGA implementation was accomplished using MathWorks HDL Coder @inc_hdl_2024 due to ease of implementation and ease of testing. Further, using the _Hardware Software Codesign_ @the_mathworks_hardware-software_2024 methodology, both the transceiver interface and PS-PL interface bindings could be automatically generated.

A reference DVB-S2 HDL Coder implementation by MathWorks @mathworks_dvb-s2_2024 was selected to reduce technical risk, as it was pretested, allowing faster development of the rest of the system. Further logic was added around this block to manage the AXI-Stream interface and to convert the output for the 12 bit DAC interface. The transmitter portion is shown in @img:hdlcoder-imp. The roll-off factor was selected as 0.25 to balance throughput with filter complexity. This resulted in a symbol rate of 20,000 / second.
...
MATLAB code was created to generate synthetic AXI packets to test the system in Simulink. The resulting output spectrum was shown to be passing against the #acr("ITU") out of band emissions mask for the amateur and amateur satellite service @noauthor_unwanted_2024 as shown in @img:spectrum. 
#figure(
  image("FIgures/pspect.png"),
  caption: "Power spectrum at transmitter output. Note that the power scale is not representative of the actual output power. The shaded area shows the ITU emission mask, where green indicates passing."
) <img:spectrum>

The resource of the implemented Vivado design is shown by subsystem and resource type in @img:util and @tab:util. This design is intended to be expanded in the future to include the payload signal processing, as well as the receive chain, therefore, excessive resource usage could cause issues. 

Overall, the highest resource usage was in the block RAM, where this design used 89% of those available. This was used for three buffers within the design, 24 for the implemented interfacing logic, a further 25 for the frame during error correction, and 75.5 for the physical layer framing. The high usage is primarily due to the entire frame being stored at once at each stage. Large optimisations could be made if the frames were stored and generated in chunks, perhaps at a higher clock speed to ensure that the output does not stall.

The other resource usage rates were deemed acceptable, the multiplexer and DSP48 tile usage was particularly low and should  not impact further designs, however, the slice #acr("LUT") usage is quite high overall with only 34.9% free.

#figure(
  image("FIgures/util.png"),
  caption: "Percentage utilisation by resource type and subsystem. The DVB-S2 transmitter is shown in pink, the processing system and programmable logic in blue, and the transceiver controller in green."
) <img:util>


Following this, a timing report was generated, finding a worst negative slack of 0.126 ns, a worst hold slack of 0.016 ns and a worst pulse width slack of 0.264 ns at 61.44 MHz. Although the design met timing, the large chip area used due to high block RAM utilisation could cause issues if higher clock speeds are required in the future. 

A further power report was generated using the Vivado power analysis tool to estimate the efficiency of the final design. The tool was used in maximum usage mode. A range of default toggle rates were used to identify the expected power draw of the design based on its activity level, as shown in @tab:power. This does not include the expected power required for the transceiver, or power amplifier components. These are early stage measurements and specific power optimisations have not been implemented yet. Further optimisation and measurement of the implemented system on hardware is required. 
...
#figure(
  ...
  caption: "Block diagram of HDL Coder implementation. Highlighted in cyan is the transmitter block created by MathWorks. Highlighted in green is the implemented blocks for communication with the processing system.",
) <img:hdlcoder-imp>
])
...
The system is still to be tested in hardware and the packet handling software is still under development. The link calculations were made using several assumptions about the ground station receiver and will be revised when measurements can be made which may impact the transmitter design requirements. 