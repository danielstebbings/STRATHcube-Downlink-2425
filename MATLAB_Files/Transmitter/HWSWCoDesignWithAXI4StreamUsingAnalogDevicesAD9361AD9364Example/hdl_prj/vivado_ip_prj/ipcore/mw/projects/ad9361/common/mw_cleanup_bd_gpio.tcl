...
if { $hasLEDs || $hasDIPs || $hasPBs } {
    ...
    if { $boardName == "zed" } {
		...
		if { $ioSum == 13 } {
		    ...
		    set Str1 "   assign gpio_bd_o[6:0] = gpio_o[19:13];"
            set Str2 " "
        } elseif { $ioSum == 10 } {
            set Str1 " "
            set Str2 "   assign gpio_i[7:0] = gpio_bd_i[7:0];"
        } elseif { $ioSum == 9  } {
            set Str1 "   assign gpio_bd_o[6:0] = gpio_o[19:13];"
            set Str2 "   assign gpio_i[7:0] = gpio_bd_i[7:0];"
        } elseif { $ioSum == 4  } {
            set Str1 " "
            set Str2 "   assign gpio_i[12:8] = gpio_bd_i[12:8];"
        } elseif { $ioSum == 3  } {
            set Str1 "   assign gpio_bd_o[6:0] = gpio_o[19:13];"
            set Str2 "   assign gpio_i[12:8] = gpio_bd_i[12:8];"
        } elseif { $ioSum == 1  } {
            set Str1 " "
            set Str2 "   assign gpio_i[12:0] = gpio_bd_i;"
        }  
    } 
    ...
    if { $hasLEDs } { 
        if { $boardName == "zcu102" } {
            set ledStr "    .GPLEDs (gpio_bd_o[$lastLED:$firstLED]),\n"
        } else {
    ...
    if { $hasDIPs } { 
        if { $boardName == "zcu102" } {
            set dipStr "    .DIPSwitches (gpio_bd_i[$lastDIP:$firstDIP]),\n" 
        } else {
    ...
    if { $hasPBs } {
        if { $boardName == "zcu102" } {
            set pbStr "    .PushButtons (gpio_bd_i[$lastPB:$firstPB]),\n"
        } else {
    ...
    while {[gets $topF line] >= 0} {
        if { [string first "system_wrapper i_system_wrapper" $line] != -1 } {
            ...
            set foundIOBuf 0
        } elseif { [string first "gpio_bd_o[6:0]" $line] != -1  } {
            puts $newTop $Str1 
        } elseif { [string first "gpio_i[12:0]" $line] != -1  } {
            puts $newTop $Str2 
        } else {
            puts $newTop $line
        }
