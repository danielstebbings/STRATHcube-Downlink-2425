...
proc ps_common_dma_config {config} {
    ...
    set_property -dict [list CONFIG.NUM_SI {1}] $ic
    ...
    dict set config "ic_mm_port" ${ic_name}/S00_AXI
...
proc setup_dma {config} {
    ...
    if {$direction == "mm2s"} {
        set MM_port m_src_axi
        set MM_dir src
    } else {
        set MM_port m_dest_axi
        set MM_dir dest
    }

    set dma [mw_add_axi_dma axi_dma_${direction}]
    set dma_name [get_property Name $dma]

    # Configure the DMA Engine
    if {$direction == "mm2s"} {
        set_property -dict [list CONFIG.DMA_DATA_WIDTH_SRC {32} ] $dma 
        set_property -dict [list CONFIG.DMA_DATA_WIDTH_DEST {32} ] $dma
        set_property -dict [list CONFIG.DMA_TYPE_DEST {1} ] $dma 
        set_property -dict [list CONFIG.DMA_TYPE_SRC {0} ] $dma     
    } else {
        set_property -dict [list CONFIG.DMA_DATA_WIDTH_SRC {32} ] $dma 
        set_property -dict [list CONFIG.DMA_DATA_WIDTH_DEST {32} ] $dma
        set_property -dict [list CONFIG.DMA_TYPE_DEST {0} ] $dma 
        set_property -dict [list CONFIG.DMA_TYPE_SRC {1} ] $dma   
    }
    ...
    mw_connect_pin $m_clk ${dma_name}/m_${MM_dir}_axi_aclk
    mw_connect_pin $m_rstn ${dma_name}/m_${MM_dir}_axi_aresetn

    # Connect the AXIS clock
    set dir_char [string index $direction 0]
    mw_connect_pin $m_clk ${dma_name}/${dir_char}_axis_aclk
